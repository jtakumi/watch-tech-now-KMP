...
技術記事プラットフォーム（Zenn / Qiita）の記事を 1 つのアプリで横断的に閲覧することを目的としています。UI とロジックは `commonMain` に集約し、Android / iOS で共通の Compose Multiplatform 画面を共有します。

想定している主な機能:

- Zenn と Qiita の記事を統合した一覧表示
- 記事の詳細閲覧
- プラットフォームやキーワードによる絞り込み・検索

> **現在の状態**: プロジェクトはひな形（初期スキャフォールド）の段階で、`App()` はタイトルと説明テキストを表示するのみです。上記の機能はこれから実装していきます。
